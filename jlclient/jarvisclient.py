...
from .httpclient import get, get_base_url, post
...
def _resolve_region(instance_type, gpu_type, num_gpus):
    ...
    candidates = [
        server for server in meta.get("server_meta", []) if server.get("gpu_type") == gpu_type and server.get("region")
    ]
    if not candidates:
        return fallback_region
...
class Instance:
    ...
    def resume(
        ...
        try:
            ...
            resume_resp = post(
                resume_req, f"templates/{self.template}/resume", jarvisclient.token, base_url=get_base_url(self.region)
            )
    ...
    def get_instance_details(machine_id, region=DEFAULT_REGION):
        ...
        for _ in range(max_attempts):
            machine_status_response = get("users/fetch", jarvisclient.token, base_url=get_base_url(region))

            matching_instances = [
                instance
                for instance in machine_status_response["instances"]
        ...
        raise InstanceCreationException(f"Timed out while waiting for machine_id={machine_id} to reach Running.")
    ...
    def create(
        ...
        resolved_region = (
            region if region else _resolve_region(instance_type=instance_type, gpu_type=gpu_type, num_gpus=num_gpus)
        )
        req_data = {
            "hdd": storage,
            "name": name,
        ...
        try:
            _validate_template_region_request(
                template=template, instance_type=instance_type, gpu_type=gpu_type, region=resolved_region
            )
            if instance_type == "gpu":
                _validate_europe_nebius_request(
                    region=resolved_region, gpu_type=gpu_type, num_gpus=num_gpus, storage=storage
                )
            elif resolved_region == EUROPE_REGION:
                raise ValueError("europe-01 supports only H100/H200 GPU requests.")

            resp = post(
                req_data, f"templates/{template}/create", jarvisclient.token, base_url=get_base_url(resolved_region)
            )
...
class User:
    ...
    def get_instances(cls) -> list[Instance]:
        resp = get("users/fetch", jarvisclient.token)
        instances = []
        for instance in resp["instances"]:
            inst = Instance(
                hdd=int(instance["hdd"]),
    ...
    def get_templates(cls):
        templates = get("misc/frameworks", jarvisclient.token)
        return {"templates": [template["id"] for template in templates["frameworks"]]}

    @classmethod
    def get_balance(cls):
        return get("users/balance", jarvisclient.token)
...
class FileSystem:
    def list(self):
        return get("filesystem/list", jarvisclient.token)

    def create(self, fs_name, storage):
        return post({"fs_name": fs_name, "storage": storage}, "filesystem/create", jarvisclient.token)

    def delete(self, fs_id):
        return post({}, "filesystem/delete", jarvisclient.token, query_params={"fs_id": fs_id})
